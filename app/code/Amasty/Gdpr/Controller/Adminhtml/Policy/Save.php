...
class Save extends AbstractPolicy
{
    ...
    public function execute()
    {
        if ($data = $this->getRequest()->getPostValue('policy')) {
            try {
                if (isset($data['id'])) {
                    ...
                    if (!$this->validateModel($data)) {
                        $this->messageManager->addErrorMessage(
                            __('Sorry, privacy policy with the same version already exists.')
                        );
                ...
                if (!$this->validateModel($data)) {
                    $this->messageManager->addErrorMessage(__('Privacy Policy - Sorry, privacy policy with the '
    ...
    private function validateModel($policyData) {
        $versions = $this->policyResource->getAllValueFromColumnPolicy(PolicyInterface::POLICY_VERSION);
        foreach ($versions as $version) {
            if (version_compare($version, $policyData['policy_version'], '=')) {
                return false;
            }
        }
